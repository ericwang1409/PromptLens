...
export function ChatDataBrowser() {
  ...
  const enrichedData = dashboardData
    ? dashboardData.prompts.map((prompt) => {
        const response = dashboardData.responses.find(
  ...
  const filteredData = enrichedData
    .filter((item) => {
      ...
      const matchesCategory =
        selectedCategory === "all" || item.prompt.category === selectedCategory;

      // Basic user filter
      const matchesUser =
        selectedUser === "all" || item.user?.id === selectedUser;
      ...
      if (advancedFilters) {
        ...
        if (advancedFilters.dateRange.from || advancedFilters.dateRange.to) {
          ...
          if (
            ...
            itemDate < advancedFilters.dateRange.from
          ) {
            matchesAdvanced = false;
          }
          if (
            advancedFilters.dateRange.to &&
            itemDate > advancedFilters.dateRange.to
          ) {
            matchesAdvanced = false;
          }
        }
        ...
        if (
          ...
          !advancedFilters.categories.includes(item.prompt.category)
        ) {
          matchesAdvanced = false;
        }
        ...
        if (
          ...
          !advancedFilters.users.includes(item.user?.id)
        ) {
          matchesAdvanced = false;
        }
        ...
        if (
          tokens < advancedFilters.tokenRange[0] ||
          tokens > advancedFilters.tokenRange[1]
        ) {
          matchesAdvanced = false;
        }
        ...
        if (
          ...
          !advancedFilters.departments.includes(item.user?.department)
        ) {
          matchesAdvanced = false;
        }
    ...
    .sort((a, b) => {
      switch (sortBy) {
        case "newest":
          return (
            new Date(b.prompt.timestamp).getTime() -
            new Date(a.prompt.timestamp).getTime()
          );
        case "oldest":
          return (
            new Date(a.prompt.timestamp).getTime() -
        ...
        case "rating":
          return (
            (b.prompt.metadata.satisfaction_rating || 0) -
            (a.prompt.metadata.satisfaction_rating || 0)
          );
        case "tokens":
          return (
            (b.response?.tokens_used || 0) - (a.response?.tokens_used || 0)
          );
        default:
          return 0;
      }
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <AdvancedFilters
            ...
            availableUsers={dashboardData?.users || []}
          />
          ...
          >
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-foreground pl-4 text-pretty leading-relaxed">
                  {prompt.content}
                </p>
              ...
              {response && (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground pl-4 text-pretty leading-relaxed">
                    {response.content}
                  </p>
                  <div className="flex gap-4 pl-4 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {response.response_time > 0
                        ? `${(response.response_time / 1000).toFixed(2)}s`
                        : "No time data"}
                    </span>
                    <span className="flex items-center gap-1">
                      <Star className="w-3 h-3" />
                      {prompt.metadata.satisfaction_rating
                        ? `${prompt.metadata.satisfaction_rating}/5`
                        : "No rating"}
                    </span>
                  </div>
                </div>
